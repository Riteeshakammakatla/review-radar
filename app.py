...
# ---------------------------------------------------------------------------
# LAZY model loading — the model is NOT loaded at startup.
# It only loads when the user actually uploads a file for the first time.
# This prevents Render from crashing on boot.
# ---------------------------------------------------------------------------
_model = None
...
# Max reviews to process on Render Free Tier (512MB RAM, 30s timeout)
MAX_ROWS = 500
...
def upload():
    ...
    try:
        ...
        if original_count > MAX_ROWS:
            message += f' (using first {MAX_ROWS} of {original_count} reviews)'
